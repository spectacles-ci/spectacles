...
def test_query_with_one_dimension_should_not_divide(
    ...
    query = Query(explore=explore, fields=(dimension,), errored=True)
    with pytest.raises(ValueError):
        next(query.divide())
...
def test_query_should_not_divide_if_not_errored(
    ...
    query = Query(explore=explore, fields=(dimension, dimension))
